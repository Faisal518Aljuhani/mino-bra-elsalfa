...
router.post('/register',
  ...
  async (req, res) => {
    ...
    try {
      ...
      const verifyExpires = Date.now() + 30 * 60 * 1000; // 30 دقيقة

      const result = db.prepare(`
        INSERT INTO users (username, email, password_hash, verify_token, verify_token_expires)
        VALUES (?, ?, ?, ?, ?)
      `).run(username, email, passwordHash, verifyToken, verifyExpires);

      try {
        await sendVerificationEmail(email, username, verifyToken);
      } catch (mailErr) {
        console.error('فشل إرسال بريد التفعيل:', mailErr.message);
        return res.status(201).json({
          message: 'تم إنشاء الحساب لكن فشل إرسال بريد التفعيل. تأكد من إعدادات SMTP في .env',
          userId: result.lastInsertRowid
        });
      }

      res.status(201).json({ message: 'تم إنشاء الحساب! تفقد بريدك الإلكتروني لتفعيله.' });
    } catch (err) {
      console.error(err);
...
router.post('/forgot-password',
  ...
  async (req, res) => {
    ...
    // نرد بنفس الرسالة سواء موجود أو لا، عشان ما نكشف حسابات
    if (user) {
      ...
      try {
        await sendResetEmail(email, user.username, resetToken);
      } catch (e) {
        console.error('فشل إرسال بريد الاستعادة:', e.message);
      }
    }

    res.json({ message: 'لو البريد مسجل عندنا، بيوصلك رابط إعادة التعيين' });
  }
);
